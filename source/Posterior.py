...
class Posterior:
    ...
    def __init__(self, inversion, alpha, data, **kwargs):
        ...
        - grid_t: the time discretization for which we are measuring (defaults to
          drone.grid_t if not provided)
        - flightpath: the flightpath for the provided alpha and time
          discretization (gets computed if not provided)
        - states: the forward states computed for the parameter basis vectors
          (will get computed if not provided)
        """
    ...
    def get_para2obs(self):
        ...
        @return:
        """
        # TODO: generalize to non-unit-vector basis for compatibility with
        # parameter reduction setting
        ...
        for i in range(self.prior.n_parameters):
            ...
            G[:, i], flightpath, _, _ = self.inversion.apply_para2obs(
    ...
    def d_invPostCov_d_control(self):
        ...
        if len(self.covar_inv_derivative[0].shape) == 0:
            ...
            # correctly
            raise RuntimeError(
                f"invalid shape = {self.covar_inv_derivative[0].shape} for d_invPostCov_d_speed"
            )
