...
class InverseProblem:
    ...
    def __init__(self, fom: FOM, drone: Drone) -> None:
        ...
        # TODO: right now it's somewhat unclear what the noise model actually is
        # -- we are just using the mass matrix.
    ...
    def set_noise_model(self, c_scaling, c_diffusion, grid_t):
        ...
        (only needed if varying from default c_scaling=1, c_diffusion=1, grid_t=drone.grid_t)
    ...
    def sample_noise(self, n_samples: int = 1) -> np.ndarray:
        ...
        #  needed. However, it's a very nice show and helps visualizing the
        #  data a lot, so ... implement it!
        raise NotImplementedError(
            "InverseProblem.sample:"
            + " still need to check how exactly we are setting up the noise model"
        )
    ...
    def compute_posterior(self, alpha, data=None, flightpath=None, grid_t=None):
        """!
        Computes the posterior distribution for given flight path parameters and
        measurements obtained along the flight. If no data is provided, the
        returned posterior will only contain the posterior covariance matrix,
        but not the posterior mean, provided the forward model is linear.
    ...
    def compute_states(self, parameters, Basis=None):
        ...
        parameters for a given basis) and saves them in self.states
    ...
    def get_states(self):
        ...
        if self.states is None:
            print(
                "WARNING:InverseProblem.get_states was called."
                + " This means the user didn't actively precompute the states."
            )
