"""!
Full order model (FOM) parent class

Methods should be implemented in child class by the user.
"""
import numpy as np
...
class FullOrderModel:
    ...
    def set_prior(self, prior_mean: np.ndarray, prior_covar: np.ndarray) -> None:
        ...
        if prior_covar.shape != (self.n_parameters, self.n_parameters):
            raise RuntimeError(
                f"In FullOrderModel.set_prior: invalid shape {prior_covar.shape}"
                + " for prior covariance matrix (expected: "
                + f"{(self.n_parameters, self.n_parameters)})"
            )
