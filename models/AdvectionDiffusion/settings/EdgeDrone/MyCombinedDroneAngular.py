...
from Path import CombinedCircularPathSingleSpeed
...
class MyDrone(Drone):
    """
    Circular path with single speed and variable angular velocity (it can turn
    in circles at the same speed)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.path_class = CombinedCircularPathSingleSpeed
    ...
    def path(self, alpha: np.ndarray) -> CombinedCircularPathSingleSpeed:
        """
        ...
        # )
        params = 0.2 * np.ones((2, num_segments))
        params[1, :] = alpha[2:]
        # np.reshape(alpha[1:], (2, num_segments)).T
        return self.path_class(
            initial_x=self.start[0],
            initial_y=self.start[1],
            initial_heading=alpha[0],
            speed=alpha[1],
            angular_velocities=alpha[2:],
            transition_times=np.linspace(
                self.grid_t[0], self.grid_t[-1], num_segments + 1
            ),
        )
    ...
    def d_position_d_control(
        ...
        d_velocity = derivatives["velocity"]
        d_heading = derivatives["initial heading"]

        output = np.empty(
            (2 + d_angular_velocity.shape[1], 2, len(grid_t))
        )  # should be (parameter, (x,y), time)
        output[0, :, :] = d_heading[:, :].T
        output[1, :, :] = d_velocity[:, :].T
        for i in range(d_angular_velocity.shape[1]):
            # output[2*i + 1, :, :] = d_velocity[:, i, :].T
            # output[2*i + 2, :, :] = d_angular_velocity[:, i, :].T
            # output[i + 1, :, :] = d_velocity[:, i, :].T
            output[i + 2, :, :] = d_angular_velocity[:, i, :].T
