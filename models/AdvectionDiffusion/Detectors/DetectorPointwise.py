...
class DetectorPointwise(Detector):
    ...
    the form:
    d(t) = u(p(t), t)
    where p(t) is the position of the drone at time t. Compared to the other
    convolution-type measurements, this is the cheapest way to compute
    measurements. However, in infinite-dimensional function space, the point
    evaluation is not necessarily well defined.
    """
    ...
    def compute_convolution(self, state: "State"):
        ...
        warnings.warn(
            "For pointwise measurements, measurements are taken from the un-convolved state."
        )
        return state

    def measure_at_position(self, pos, t, state):
        """Get measurement at a single position and time"""
        try:
