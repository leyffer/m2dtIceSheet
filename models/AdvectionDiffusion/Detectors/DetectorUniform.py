...
class DetectorUniform(Detector):
    ...
    where C(t) is a circle of a given radius (input parameter) around the
    position of the drone at time t.
    """

    center = np.array([0.75 / 2, 0.55 / 2])

    def __init__(self, grid_t=None, radius: float = 0.05, **kwargs):
    ...
    def measure_at_position(
        ...
        To compute the measurements on the subdomain B(t), we proceed the
        following way:
        1) We construct a reference mesh of a circular domain
        2) We use the shift of the circle's mesh point from the center to
           evaluate the state at the drone position + shift. This way, we
           project the values of the state in the circle around the drone
           position onto the reference circle.
        3) We use the reference mesh and the projected values to compute the
        ...
        - We don't need to track the circle on the whole domain Omega. In
          particular, for evaluating the integral, FEniCS doesn't need to
          iterate over all points in Omega to find the subdomain. It's a lot
          faster.
