...
class DetectorGaussian(Detector):
    ...
    Unfortunately, computing the convolution over the whole domain at each time
    step is very expensive. We use the "Weierstrass trick" to approximate the
    convolution (explained in compute_convolution below).

    Note that currently it is only an approximation because we are not setting
    the correct boundary conditions. These, in turn, would again require
    evaluating the convolution everywhere.
    """
    ...
    def __init__(self, fom, sigma:float=0.1, **kwargs):
    ...
    def compute_convolution(self, state:"State"):  # -> myState:
        r"""
        In this function we approximate the convolution of the state $v$ with
        the multivariate Gaussian $Psi$,
        ...
        $\dot{u} = \Delta u$
        for initial condition u(x,0) = \delta_0(x). From linearity of the heat
        equation, we get that for initial condition
        $u(x,0)=g$, we'll have
        $u(x, t) = \int \Phi(x-y, t) g(y) dy$.

        In our case we want to compute
        $\int Psi(x-y) v(y) dy$
        where $v$ is our state, and $Psi$ the multivariate Gaussian defined
        above. We can obtain this convolution by solving the heat equation with
        t=0.5, $k=\sigma^2$.

        There is one caveat to this procedure, which is that Phi was the
        solution on the infinite-dimensional domain, but our state is typically
        defined on a bounded domain $\Omega$. The correct boundary condition to
        apply on a bounded domain would require evaluating the convolution that
        we want to get in the first place. At least, I didn't find a way around
        yet.

        In the code below we are applying 0-Neumann boundary conditions. That's
        not correct, so the state we obtain is going to be an approximation.
        However, as long as sigma is small enough and we are evaluating the
        convolution sufficiently far away from the boundary, then it shouldn't
        be too bad.
        ...
        @return:
        """
        # TODO: the code below does not apply the correct boundary conditions.
        #  If anything, this is an approximation at best.

        # check if this convolution has already been computed before, use
        # self.eval_mode as unique identifier
        ...
        if state.bool_is_transient:
            raise NotImplementedError(
                "In DetectorGaussian.compute_convolution: time-dependent convolution with Gaussian not implemented yet"
            )

        # set diffusion coefficient to be the variance we want
        diffusion = self.sigma**2

        # choose the final time to bring the coefficient 1/(4*pi*D*t) into the
        # form 1/(2*pi*sigma**2)
        t_final = 0.5
        ...
        F = (
            dl.inner(u, v) * dl.dx
            + dl.Constant(dt * diffusion)
            * dl.inner(dl.nabla_grad(u), dl.nabla_grad(v))
            * dl.dx
        )
        A = dl.assemble(F)
        ...
        # we are saving the convolution in myState-format such that we can
        # easily access its derivative in the future without repeat computations
    ...
    def measure(self, flight, state) -> np.ndarray:
        ...
        1) we compute the convolution of the state with the Gaussian everywhere
            in the domain. This step is outsourced to compute_convolution. We
            save the convolution as a field, so we never need to compute it
            again.
        ...
        for k in range(n_steps):
            ...
            try:
                ...
                warnings.warn(
                    "DetectorGaussian.measure: flightpath goes outside of computational domain"
                )
                data[k] = 0.0
        ...
        for i in range(grid_t.shape[0]):
            ...
            if state.bool_is_transient:
                ...
                try:
                    ...
                    warnings.warn(
                        "DetectorGaussian.d_measurement_d_position: flightpath goes outside of computational domain"
                    )
